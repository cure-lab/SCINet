...
class extend_SCINet(nn.Module):
    def __init__(self, output_len, input_len, input_dim = 9, hid_size = 1, num_stacks = 1,
                num_levels = 3, concat_len = 0, groups = 1, kernel = 5, dropout = 0.5,
                 single_step_output_One = 0, input_len_seg = 0, positionalE = False, modified = True, RIN=False):
        super(extend_SCINet, self).__init__()
        ...
        self.trend1 = nn.Linear(input_len,input_len)
        self.trend2 = nn.Linear(input_len,input_len)
        self.blocks1 = EncoderTree(
            in_planes=self.input_dim,
            num_levels = self.num_levels,
            kernel_size = self.kernel_size,
        ...
        self.projection1 = nn.Conv1d(self.input_len, self.output_len, kernel_size=1, stride=1, bias=False)
        self.recon_projection1= nn.Conv1d(self.input_len, self.input_len, kernel_size=1, stride=1, bias=False)

        self.projection2 = nn.Conv1d(self.input_len, self.output_len, kernel_size=1, stride=1, bias=False)
        self.recon_projection2= nn.Conv1d(self.input_len, self.input_len, kernel_size=1, stride=1, bias=False)

        self.dropout_layer=nn.Dropout(self.dropout)
        self.relu=nn.LeakyReLU()

        self.concat_fusion1=nn.Conv1d(self.input_len*2, self.input_len, kernel_size=1, stride=1, bias=False)
        self.concat_fusion2=nn.Conv1d(self.input_len*2, self.input_len, kernel_size=1, stride=1, bias=False)
    ...
    def forward(self, x):
        ...
        pred1 = self.projection1(x)
        recon1=self.recon_projection1(x)
        ...
        pred2=self.projection2(x)
        recon2=self.recon_projection2(x)
        y_hat=pred1+pred2
        x_hat=recon1+recon2

        if self.RIN:
            x_hat = x_hat - self.affine_bias
            x_hat = x_hat / (self.affine_weight + 1e-10)
            x_hat = x_hat * stdev
            x_hat = x_hat + means

            y_hat = y_hat - self.affine_bias
            y_hat = y_hat / (self.affine_weight + 1e-10)
            y_hat = y_hat * stdev
            y_hat = y_hat + means


        return y_hat,x_hat, pred1,recon1
